...
import telemetry.domain.TelemetryDataF12019Impl;
import telemetry.domain.TelemetryDataF12019Impl.*;
...
@Repository
@Log4j
public class UdpRepositoryF12019MotionToFR2017Impl implements Runnable {
	@Autowired
	private UdpServer udpServer;
	@Autowired
    private TelemetryDataF12019Impl f12019;
	...
	private void addHeaderData(final TelemetryDataF12017Impl f12017, final F12019Header header) {
	...
	public void run() {
		...
		while (true) {
			try (final DatagramSocket datagramSocket = new DatagramSocket(udpListenPort)) {
                datagramPacket = new DatagramPacket(new byte[TelemetryDataF12019Impl.LARGEST_PACKET_SIZE], TelemetryDataF12019Impl.LARGEST_PACKET_SIZE);
                datagramSocket.setSoTimeout(5000);
                datagramSocket.receive(datagramPacket);
                waiting = false;
                byte[] data = datagramPacket.getData();
                if (0 == f12019.getPacketId(data)) { //Packetid = motion (0)
                    final MotionDataPacket motion = f12019.getMotionData(data);
                ...
                } else if (2 == f12019.getPacketId(data)) { //Lapdata packet
                    final LapDataPacket allCarsLapData = f12019.getLapData(data);
                ...
                } else if (6 == f12019.getPacketId(data)) { //Packetid = car telemetery
                    final CarDataPacket carsData = f12019.getCarData(data);
                ...
                if (receivedCarData && receivedMotionData && recievedLapData) {
                    ...
                    if (!processingData) {
                        processingData = true;
                        System.out.println("\nReceiving F12019 UDP data on port " + udpListenPort + ", transmitting converted F12017 data to ports: " + proxyPorts);
                    }
                ...
                if(!waiting) {
                    System.out.print("\nWaiting to receive F1 2019 UDP data on port " + udpListenPort);
                    waiting = true;
                } else {
                    System.out.print(".");
                }
